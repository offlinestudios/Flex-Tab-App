...
interface WorkoutTimerProps {
  ...
  totalVolume: number;
  onEnd: () => void;
  /** Called when the user taps "Finish Workout" — triggers workout finalization and the share screen.
   *  Receives the formatted duration string, e.g. "36:12" */
  onFinishAndShare?: (duration: string) => void | Promise<void>;
}
...
export function WorkoutTimer({ isActive, exerciseCount, totalSets, totalVolume, onEnd, onFinishAndShare }: WorkoutTimerProps) {
  ...
  // Timestamp-based approach: instead of incrementing a counter every second
  // (which iOS throttles when the screen is locked or the tab is backgrounded),
  // we store the wall-clock moment the timer last started and accumulate elapsed
  // seconds on top of any previously banked time.
  const startTimeRef = useRef<number | null>(null); // Date.now() when last resumed
  ...
  useEffect(() => {
    ...
    if (!paused) {
      // Resuming — record the current wall-clock time
      startTimeRef.current = Date.now();
      intervalRef.current = setInterval(recalc, 500); // 500 ms for smooth display

      // When the user returns from lock screen / another app, recalculate immediately
      const onVisible = () => { if (!document.hidden) recalc(); };
      ...
      // Pausing — bank the elapsed seconds so far
  ...
  useEffect(() => {
    if (!isActive) {
      // Workout ended — reset everything
      setSeconds(0);
      setPaused(false);
          ...
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "var(--card)", borderRadius: 24, width: "calc(100% - 48px)", maxWidth: 380, padding: "28px 24px 24px", zIndex: 101, animation: "slideIn .3s ease" }}>
            ...
            <button
              onClick={async () => {
                ...
                if (onFinishAndShare) {
                  await onFinishAndShare(durationStr);
                }
                onEnd();
              }}
